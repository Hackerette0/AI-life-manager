...
export default function WellnessReminders({ reminders, onUpdate }: Props) {
  ...
  return (
    <div className="glass-card rounded-lg overflow-hidden">
      {/* Active alert */}
      {alert && (
        <div className="bg-tertiary/[0.06] border-b border-tertiary/20 p-5 animate-slide-up">
          <div className="flex items-start gap-4">
            <div className="w-11 h-11 bg-tertiary/10 rounded-full flex items-center justify-center border border-tertiary/25 shrink-0">
              <span
            ...
            <div className="flex-1">
              <p className="font-headline text-xl italic text-white">{alert.label}</p>
              <p className="text-sm text-white/50 mt-0.5">{messages[alert.type]}</p>
              <div className="flex flex-wrap gap-2 mt-3">
                <button
                  onClick={dismiss}
                  className="glimmer-btn px-4 py-1.5 bg-tertiary/20 border border-tertiary/30 text-tertiary rounded-full font-label text-[9px] uppercase tracking-widest interactive-scale"
                >
                  ✓ Done
                </button>
                {[15, 30].map((m) => (
                  <button
                    key={m}
                    onClick={() => snooze(m)}
                    className="px-4 py-1.5 glass-dark border border-white/15 text-white/50 rounded-full font-label text-[9px] uppercase tracking-widest interactive-scale hover:text-white/70"
                  >
                    Snooze {m}m
                  </button>
                ))}
      ...
      >
        <div className="flex items-center gap-3">
          <span
            className="material-symbols-outlined text-secondary/70 text-xl"
            style={{ fontVariationSettings: "'FILL' 1" }}
          >
            notifications_active
          </span>
          <div className="text-left">
            <p className="font-headline text-xl italic text-white">Wellness Reminders</p>
        ...
        <span
          className={`material-symbols-outlined text-white/20 transition-transform duration-300 ${expanded ? "rotate-180" : ""}`}
        >
          expand_more
        </span>
      </button>
      ...
      {/* Reminder rows */}
      {expanded && (
        <div className="border-t border-white/[0.08] divide-y divide-white/[0.05] animate-fade-in">
          {reminders.map((r) => {
            const last   = r.lastTriggered ? new Date(r.lastTriggered).getTime() : 0;
            ...
            return (
              ...
              >
                <div className={`w-9 h-9 rounded-full flex items-center justify-center border shrink-0 ${
                  ...
                  <span
                    className={`material-symbols-outlined text-lg ${r.enabled ? "text-secondary" : "text-white/30"}`}
                    style={{ fontVariationSettings: "'FILL' 1" }}
                  >
