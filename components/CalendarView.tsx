    ...
    <div className="bg-[#1A1A1A] rounded-2xl border border-[#2A2A2A] shadow-sm overflow-hidden">
      ...
      <div className="flex items-center justify-between p-5 border-b border-[#2A2A2A]">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-[18px] font-bold text-[#FFFDE7]">Today&apos;s Calendar</h3>
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="interactive-scale p-2 rounded-full glass-dark border border-[#2A2A2A] text-[#AA7790] hover:text-[#FF9FCA] transition-all"
          ...
          <button
            ...
            className={`interactive-scale flex items-center gap-1.5 px-4 py-2 rounded-full text-[10px] font-semibold uppercase tracking-widest font-bold transition-all ${
              showForm
                ? "glass-dark border border-[#2A2A2A] text-[#FF9FCA]"
      ...
      {showGcalInfo && (
        <div className="mx-5 my-3 bg-[#0C0C0C] rounded-xl border border-[#FF3D9A]/20 p-4 animate-slide-up">
          <div className="flex items-start gap-3">
            <span className="material-symbols-outlined text-[#FF3D9A] text-xl shrink-0 mt-0.5" style={{ fontVariationSettings: "'FILL' 1" }}>
              info
            </span>
            <div className="space-y-2">
              <p className="text-[10px] font-semibold uppercase tracking-widest text-[#FF3D9A]">Google Calendar Integration</p>
              <p className="text-xs text-[#FF9FCA] leading-relaxed">
                To sync Google Calendar events, add your Google Calendar API credentials to <code className="text-[#FF3D9A] bg-[#141414] px-1 rounded">.env.local</code>:
              </p>
              <div className="bg-[#0C0C0C] rounded p-2.5 font-mono text-[10px] text-[#AA7790] space-y-0.5">
              ...
              <p className="text-xs text-[#AA7790] leading-relaxed">
                Then visit <span className="text-[#FF3D9A]">console.cloud.google.com</span>, enable the Calendar API, and create OAuth 2.0 credentials. Until then, add events manually below.
              </p>
            </div>
          </div>
        </div>
      ...
      {/* Add event form */}
      {showForm && (
        <div className="mx-5 my-3 bg-[#0C0C0C] rounded-xl border border-[#2A2A2A] p-4 space-y-3 animate-slide-up">
          <input
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <p className="text-[10px] font-semibold uppercase tracking-widest text-[#AA7790] mb-1.5">Start time</p>
              <input
                type="time" value={startTime} onChange={(e) => setStartTime(e.target.value)}
                className="w-full bg-[#141414] border border-[#2A2A2A] rounded-lg px-3 py-2 text-sm text-[#FF9FCA] focus:outline-none focus:border-[#FFD60A]/40 "
              />
            </div>
            <div>
              <p className="text-[10px] font-semibold uppercase tracking-widest text-[#AA7790] mb-1.5">End time</p>
              <input
          ...
          <input
            ...
            className="w-full bg-[#141414] border border-[#2A2A2A] rounded-lg px-4 py-2.5 text-sm text-[#FF9FCA] placeholder-[#C6C6C8] focus:outline-none focus:border-[#FFD60A]/40 transition-all"
          />
          {/* Color picker */}
          <div>
            <p className="text-[10px] font-semibold uppercase tracking-widest text-[#AA7790] mb-2">Color</p>
      ...
      {allDayEvents.length > 0 && (
        <div className="px-5 pt-3 pb-1 flex flex-wrap gap-2">
          {allDayEvents.map((e) => (
            <span
              key={e.id}
              className="text-[10px] font-semibold uppercase tracking-widest px-3 py-1.5 rounded-full border text-[#FF9FCA]"
      ...
      <div className="p-5">
        {todayEvents.length === 0 && planBlocks.length === 0 ? (
          <div className="py-8 text-center">
            ...
            <p className="text-[10px] font-semibold uppercase tracking-widest text-[#AA7790]">No events yet — add one above</p>
            <p className="text-xs text-[#AA7790] mt-1">
          ...
          <div className="space-y-2">
            ...
            <div className="flex items-center gap-2 mb-3">
              ...
              <p className="text-[10px] font-semibold uppercase tracking-widest text-[#FFD60A]">
                ...
                return (
                  ...
                  >
                    <div className="text-center w-12 shrink-0">
                      <p className="font-label text-[10px] font-bold text-[#FF9FCA]">
                        {pad(startH)}:{pad(startM)}
                      </p>
                      <p className="font-label text-[8px] text-[#AA7790]">
                        {duration}m
                      </p>
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        ...
                        <p className={`text-sm font-medium leading-snug flex-1 ${isPast ? "line-through text-[#AA7790]" : "text-white/85"}`}>
                          {item.title}
                        </p>
                        {isNow && (
                          <span className="text-[9px] font-semibold uppercase tracking-widest bg-primary text-[#FFFDE7] px-2 py-0.5 rounded-full animate-pulse shrink-0">
                            now
                          </span>
                        )}
                        {!item.isPlan && (
                      ...
                      {item.location && (
                        <p className="text-xs text-[#AA7790] mt-0.5 flex items-center gap-1">
            ...
            {dayPlan && dayPlan.schedule.length > 0 && (
              <div className="mt-4 pt-4 border-t border-white/[0.06]">
                <p className="text-[10px] font-semibold uppercase tracking-widest text-[#AA7790] mb-2">AI Schedule</p>
                <div className="space-y-1.5">
                  {dayPlan.schedule.map((b, i) => {
                    ...
                    return (
                      ...
                      >
                        <p className="font-label text-[10px] font-bold text-[#AA7790] w-10 shrink-0">{b.time}</p>
                        <p className={`text-xs leading-snug ${isPast ? "line-through text-[#AA7790]" : "text-[#FF9FCA]"}`}>
                          {b.title}
                        </p>
                        <p className="font-label text-[8px] text-[#AA7790] ml-auto shrink-0">{b.duration}m</p>
                      </div>
                    );
                  })}
                </div>
